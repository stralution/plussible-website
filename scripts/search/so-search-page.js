document.addEventListener("DOMContentLoaded", function () {
  ...
  resultsHeading.textContent = `Special Offers Results for: ${searchQuery}`;
  const userZipCode = localStorage.getItem('userZipCode') || 'undefined';
  zipCodeElement.textContent = `ZIP Code: ${userZipCode}`;
    ...
    paginatedOffers.forEach(offer => {
      ...
      offerCard.innerHTML = `
        ...
        <div class="right">
          ...
          <a href="#" class="engage-btn" onclick="handleEngageButtonClick('${offer.name}', '${offer.priceType}', '${offer.duration}')">Engage Service</a>
        </div>
      `;
  ...
  quickSearchButton.addEventListener('click', function (event) {
    ...
    if (results.length === 0) {
      ...
      resultsHeading.textContent = `Special Offers Results for: ${query}`;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Check if user is logged in and redirect to c-so-search-page.html if true
  const storedUserData = localStorage.getItem('currentUser');
  if (storedUserData) {
    window.location.href = 'c-so-search-page.html';
  }
});
...
function handleEngageButtonClick(offerName, priceType, duration) {
  const storedUserData = localStorage.getItem('currentUser');
  if (!storedUserData) {
    alert("You need to sign in to engage this service.");
    return;
  }

  showCalendar(offerName, priceType, duration);
}
